...
private:
    ...
    CHttpResponse* post_webrequest(string endpoint, string jwt_token, string &_data);
    CHttpResponse* post_wininet(string endpoint, string jwt_token, string &_data);

public:
    CHttpService(ENUM_SDK_PRODUCT_TYPE product_type = PRODUCT_TYPE_ROBOT);
    ~CHttpService();

    CHttpResponse* post(string endpoint, string jwt_token, string &_data);
...
//+------------------------------------------------------------------+
CHttpResponse* CHttpService::post(string endpoint, string jwt_token, string &_data)
{
    if(m_product_type == PRODUCT_TYPE_INDICATOR)
        return post_wininet(endpoint, jwt_token, _data);
    return post_webrequest(endpoint, jwt_token, _data);
}

//+------------------------------------------------------------------+
//| POST via built-in WebRequest (EAs and scripts only)              |
//+------------------------------------------------------------------+
CHttpResponse* CHttpService::post_webrequest(string endpoint, string jwt_token, string &_data)
{
    char post_data[];
    char _result[];
    ...
    StringToCharArray(_data, post_data, 0, StringLen(_data), CP_UTF8);

    CHttpResponse* response = new CHttpResponse();
    if(response == NULL) return NULL;

    int res = WebRequest("POST", m_base_url + endpoint, headers, HTTP_TIMEOUT, post_data, _result, response_headers);
    ...
    {
        response.code = res;
        response.body = CharArrayToString(_result, 0, -1, CP_UTF8);
...
//+------------------------------------------------------------------+
CHttpResponse* CHttpService::post_wininet(string endpoint, string jwt_token, string &_data)
{
    ...
    int _status = WinINetPost(m_wininet_host, full_path, m_wininet_port,
                              headers_str, _data, response_body);
